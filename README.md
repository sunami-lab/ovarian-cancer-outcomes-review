...
| `docs/existing_reviews.md` | **Has this review already been done?** PROSPERO + PubMed duplicate check. |
| `docs/screening_burden.md` | What the search will actually cost to screen, from observed deduplication ratios. |
| `docs/impact_factors.md` | Every impact factor quoted, with source and confidence. |
| `docs/validation.md` | Live PubMed run of every transcribed strategy. |
| `scripts/` | Build, and five checks — see below. |

Everything regenerable is regenerated by a script, and every script can be re-run:

```bash
pip install pyyaml openpyxl
python3 scripts/build.py                      # workbook + Markdown tables
python3 scripts/validate_pubmed.py --write    # do the transcribed strategies parse and retrieve?
python3 scripts/test_draft.py --write         # how much is there to screen?
python3 scripts/recall_test.py --write        # what does the drafted strategy miss?
python3 scripts/lint_strategies.py --write    # syntax-check the untested translations
python3 scripts/find_candidates.py --all      # re-run candidate finding for any outcome
```
...
- **Most reviews do not search on the outcome.** Population AND intervention, with the
  outcome applied at screening, is the dominant structure. Neo 2017 is the counter-example
  — no intervention concept at all, and a thirteen-synonym functional-disability block.
...
- **No review in this impact band enumerates the 12 recommended symptoms as a search
  ...
  `draft/symptom_blocks_published.yml` fills the gap from twelve single-symptom reviews.

## What the drafted strategy found out

`draft/` is not just an assembly of the blocks above; it was tested, and the test changed
the recommendation twice.

- **The recommended search retrieves 23/23 known items (100%)** — real studies a review of
  this question must not miss, drawn from trial PRO publications and from the ovarian
  reviews' own included-study lists (`draft/recall.md`).
- **The treatment block was dropped on the evidence.** It saves 32% of the screening burden
  but loses a known item, and the miss diagnoses a whole class: PRO secondary analyses of
  trials that carry no treatment MeSH and never restate the intervention.
- **The age block was dropped on the evidence.** It costs 8 of 23 known items — and they are
  precisely the trial quality-of-life papers the review exists to find.
- **The strategy fails PRESS domain 6** (`draft/press.md`): no grey literature or trial
  registry component yet. For a review of older adults, that is the most consequential gap.
- **This review is not already taken** (`docs/existing_reviews.md`), though one live
  PROSPERO registration is close enough to act on now.
